...
class MockClassifierNode(MockTreeNode):
    ...
    def classify(self, user_input: str, children: List[TreeNode], context=None) -> TreeNode:
        """Mock classification."""
        if children:
...
class TestIntentGraphNodeManagement:
    ...
    def test_add_root_node_with_validation_failure(self):
        ...
        with patch('intent_kit.graph.intent_graph.validate_graph_structure') as mock_validate:
...
class TestIntentGraphValidation:
    ...
    def test_validate_graph_success(self):
        ...
        with patch('intent_kit.graph.intent_graph.validate_node_types') as mock_validate_types, \
             patch('intent_kit.graph.intent_graph.validate_splitter_routing') as mock_validate_routing, \
             patch('intent_kit.graph.intent_graph.validate_graph_structure') as mock_validate_structure:
            
            mock_validate_structure.return_value = {"total_nodes": 1, "routing_valid": True}
    ...
    def test_validate_graph_with_validation_failure(self):
        ...
        # Mock validation to fail
        with patch('intent_kit.graph.intent_graph.validate_node_types') as mock_validate_types:
            mock_validate_types.side_effect = GraphValidationError("Node type validation failed")
    ...
    def test_validate_splitter_routing(self):
        ...
        with patch('intent_kit.graph.intent_graph.validate_splitter_routing') as mock_validate:
...
class TestIntentGraphSplitting:
    ...
    def test_call_splitter_with_context(self):
        ...
        def custom_splitter(user_input: str, debug: bool = False, context=None) -> List[IntentChunk]:
...
class TestIntentGraphRouting:
    ...
    def test_route_chunk_to_root_node_no_match(self):
        ...
        with patch('intent_kit.graph.intent_graph.classify_intent_chunk') as mock_classify:
...
class TestIntentGraphContextTracking:
    ...
    def test_log_context_changes(self):
        ...
        graph._log_context_changes(state_before, state_after, "test_node", debug=True, context_trace=False)
...
class TestIntentGraphVisualization:
    ...
    def test_render_execution_graph_no_visualization(self):
    ...
    def test_render_execution_graph_no_visualization_library(self):
