...
class TestHandlerNodeInitialization:
    ...
    def test_init_basic(self):
        ...
        def arg_extractor(user_input: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def test_init_with_context_dependencies(self):
        ...
        def arg_extractor(user_input: str, context: Dict[str, Any]) -> Dict[str, Any]:
            return {"name": user_input.split()[-1], "user_id": context.get("user_id", "unknown")}
...
class TestHandlerNodeTypeValidation:
    ...
    def test_validate_types_conversion_failure(self):
        ...
        assert result.success is False
        assert result.error is not None
        assert "invalid literal" in result.error.message.lower() or "type" in result.error.message.lower()
