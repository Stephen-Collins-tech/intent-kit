...
class TestLLMClarifier:
    ...
    def test_def test_def test_get_default_clarification_prompt(self): -> None: -> None:
    ...
    def test_def test_def test_create_llm_clarifier_basic(self): -> None: -> None:
        """Test creating a basic LLM clarifier function."""
        # Mock LLM config
        mock_llm_config = {"provider": "openrouter", "model": "google/gemma-3-27b-it"}

        # Mock LLM response
mock_response = "I need more details about your booking request. Please specify the
        destination and date."

        with patch()
            "intent_kit.services.llm_factory.LLMFactory.generate_with_config"
(        ) as mock_generate:
...
(            )
            ...
            assert ()
                result
                == mock_response
+ "\n\nPlease provide your response in the following format: Please specify:
                [destination] [date]"
(            )
            mock_generate.assert_called_once()

    def test_def test_def test_create_llm_clarifier_with_context(self): -> None: -> None:
        """Test LLM clarifier with context information."""
        mock_llm_config = {"provider": "openrouter", "model": "google/gemma-3-27b-it"}
mock_response = "Based on your previous bookings, I need more details about your flight
        request."
...
(        ) as mock_generate:
            ...
            clarifier_func = create_llm_clarifier()
                llm_config=mock_llm_config,
clarification_prompt_template="Generate a clarification for:
                {user_input}\n{context_info}",
                expected_response_format="Please specify: [details]",
(            )

            context = {
                "previous_bookings": "Paris, London",
                "user_preferences": "window seat",
            }
    ...
    def test_def test_def test_create_llm_clarifier_fallback_on_error(self): -> None: -> None:
...
(            )
    ...
    def test_def test_def test_create_llm_clarifier_node(self): -> None: -> None:
...
(            )
    ...
    def test_def test_def test_llm_clarifier_node_execution(self): -> None: -> None:
...
(            )
    ...
    def test_def test_def test_llm_clarifier_node_with_custom_prompt(self): -> None: -> None:
...
(            )
    ...
    def test_def test_def test_llm_clarifier_node_with_context(self): -> None: -> None:
...
(            )
    ...
    def test_def test_def test_llm_clarifier_node_handle_clarification_response(self): -> None: -> None:
...
(        )
        ...
        context.set()
            ...
            {"original_input": "book", "attempts": 0, "max_attempts": 2},
(        )
    ...
    def test_def test_def test_llm_clarifier_node_max_attempts(self): -> None: -> None:
...
(        )
    ...
    def test_def test_def test_llm_clarifier_node_without_expected_format(self): -> None: -> None:
...
(            )
    ...
    def test_def test_def test_llm_clarifier_node_error_structure(self): -> None: -> None:
...
(        ) as mock_generate:
            ...
            node = create_llm_clarifier_node()
                name="test_llm_clarifier", llm_config=mock_llm_config
(            )
