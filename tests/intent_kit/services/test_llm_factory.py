...
class TestLLMFactory:
    ...
    def test_create_client_missing_api_key_for_openai(self):
        ...
        with pytest.raises(ValueError, match="LLM config must include 'api_key' for provider: openai"):
            LLMFactory.create_client(llm_config)
    ...
    def test_create_client_missing_api_key_for_google(self):
        ...
        with pytest.raises(ValueError, match="LLM config must include 'api_key' for provider: google"):
            LLMFactory.create_client(llm_config)
    ...
    def test_create_client_missing_api_key_for_openrouter(self):
        ...
        with pytest.raises(ValueError, match="LLM config must include 'api_key' for provider: openrouter"):
            LLMFactory.create_client(llm_config)
    ...
    def test_generate_with_config_anthropic(self, mock_anthropic_client):
        ...
        assert result == "Generated response"
        mock_client.generate.assert_called_once_with("Test prompt", model="claude-3-sonnet")
    ...
    def test_generate_with_config_openrouter(self, mock_openrouter_client):
        ...
        assert result == "Generated response"
        mock_client.generate.assert_called_once_with("Test prompt", model="openai/gpt-4")
