...
def book_hotel(location: str, check_in: str, check_out: str, room_type: str = "standard") -> str:
    """Book a hotel with the given parameters."""
    return f"Hotel booked in {location} from {check_in} to {check_out}, {room_type} room"


def book_restaurant(name: str, date: str, time: str, party_size: int = 2) -> str:
    """Book a restaurant reservation."""
    return f"Restaurant reservation at {name} on {date} at {time} for {party_size} people"
...
def create_booking_graph():
    ...
    def booking_classifier(user_input: str, children, context=None):
        ...
        if "flight" in input_lower or "fly" in input_lower:
            ...
            return hotel_action
        elif "restaurant" in input_lower or "dinner" in input_lower or "lunch" in input_lower:
    ...
    for i, user_input in enumerate(test_cases, 1):
        ...
        if result.success is False and result.children_results:
            ...
            if clarifier_result.node_type == NodeType.CLARIFY:
                ...
                print(f"  → Message: {clarifier_result.output['clarification_message']}")
                ...
                response = clarifier_node.handle_clarification_response(clarification_response, context)
                ...
                if response["success"]:
                    ...
                    clarified_result = graph.route(response['clarified_input'], debug=True)
                    if clarified_result.success:
                        print(f"  → Final result: {clarified_result.output}")
                    else:
                        print(f"  → Still needs processing: {clarified_result.error.message if clarified_result.error else 'Unknown error'}")
                else:
                    print(f"  → Clarification failed: {response['error']}")
            else:
                print(f"  → No clarification needed, but execution failed: {clarifier_result.error.message if clarifier_result.error else 'Unknown error'}")
        else:
...
def demonstrate_clarifier_with_context():
    ...
    response2 = clarifier_node.handle_clarification_response("book a flight to Paris", context)
    ...
    response3 = clarifier_node.handle_clarification_response("book a flight to Paris tomorrow", context)
